...
class SinglyLinkedList:
    ...
    def delete(self, data):
            # ถ้าลิสต์ว่างเปล่า ลบไม่ได้แน่นอน
            if self.head is None:
                print("Cannot delete, " + data + " does not exist.")
                return

            # ถ้าตัวที่จะลบอยู่ที่หัวขบวน (Head)
            if self.head.data == data:
                self.head = self.head.next # ย้ายหัวไปที่ตัวถัดไป
                self.count -= 1
                return

            # เดินหาโหนดที่อยู่ก่อนหน้าตัวที่จะลบ
            curr = self.head
            while curr.next:
                # ถ้าโหนดถัดไปคือตัวที่จะลบ
                if curr.next.data == data:
                    # ให้โหนดปัจจุบันชี้ข้ามไปที่ตัวถัดจากตัวที่จะลบ
